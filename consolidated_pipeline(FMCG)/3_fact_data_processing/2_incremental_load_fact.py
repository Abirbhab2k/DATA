...
base_path = f's3://sportsbar-dp001/{data_source}'
...
df = df.withColumn("order_qty", col("order_qty").cast("double"))
...
# Remove duplicate rows from df_joined based on merge keys before merge
merge_keys = ["order_placement_date", "order_id", "product_code", "customer_id"]
df_joined_dedup = df_joined.dropDuplicates(merge_keys)

if not (spark.catalog.tableExists(silver_table)):
    df_joined_dedup.write.format("delta").option(
        "delta.enableChangeDataFeed", "true"
    ).option("mergeSchema", "true").mode("overwrite").saveAsTable(silver_table)
else:
    silver_delta = DeltaTable.forName(spark, silver_table)
    silver_delta.alias("silver").merge(
        df_joined_dedup.alias("bronze"),
